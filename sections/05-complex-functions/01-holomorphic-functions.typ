...
#proof[
  ...
  *Первое доказательство*, для случая, когда $(diff f)/(diff x)$ и $(diff f)/(diff y)$ непрерывно. Знаем, что замкнутость и непрерывность частной производной влечет локальную точность. Проверим замкнутость. 
...
#proof[
  ...
  Такая $g in C(Omega)$ и $g in H(Omega without {a})$. По следствию из теоремы выше про локальную точность голоморфной функции без конечного числа точек, $g$ --- локально точная, и $integral_gamma f(z) dif z = 0$.
  Тогда
  $
...
#th(name: "Морера")[
  $f in C(Omega)$. $f(z) dif z$ локально точная. Тогда $f in H(Omega)$.
]
...
#proof[
  Условие говорит о том, что $f(z) dif z$ локально точна, теореме Морера говорит, что $f in H(Omega)$.
]
...
#th[
  ...
  6. Форма $f(z) dif z$ замкнута, и частная производная $f$ непрерывна, $f in C(Omega)$
...
#proof[
  Предположим противное. Пусть $P(z) != 0$ для любого $z in CC$. Тогда $f(z) := 1/P(z) in H(C)$. Если мы проверим, что она ограничена, мы получим противоречие, так как тогда $f$ --- константа.

  Запишем многочлен в примеденном виде: $P(z) = z^n + a_(n - 1) z^(n - 1) + ... + a_1 z + a_0$. Пусть $R := 1 + abs(a_(n - 1)) + ... + abs(a_1) + abs(a_0)$. $f$ непрерывна в ${abs(z) <= R}$, значит $f$ --- ограничена в этом круге. Проверим, что $f$ ограничена в ${abs(z) >= R}$. Оценим снизу $P$:
  $
